...
const SolarSystem = () => {
  ...
  const meteoroidCanvasRef = useRef(null);
  const meteoroidRendererRef = useRef(null);
  const meteoroidAnimationRef = useRef(null);
  ...
  // 3D Meteoroid Banner Effect
  useEffect(() => {
    if (!meteoroidCanvasRef.current || isExpanded) return;

    const canvas = meteoroidCanvasRef.current;
    ...
    meteoroidRendererRef.current = renderer;

    // Lighting
    const ambientLight = new THREE.AmbientLight(0xf8d7e6, 0.6);
    ...
    const pointLight = new THREE.PointLight(0x994D74, 0.8, 50);
    pointLight.position.set(-10, 5, 10);
    scene.add(pointLight);

    // Create meteoroid geometry (irregular rock shape)
    const createMeteroidGeometry = (size, detail) => {
      const geometry = new THREE.IcosahedronGeometry(size, detail);
      const positions = geometry.attributes.position;

      // Distort vertices for irregular rock shape
    ...
    // Create meteoroid material with realistic rock appearance
    const createMeteroidMaterial = (colorVariant) => {
      const colors = [
        { base: 0x63133b, light: 0x994D74 },
        { base: 0x802754, light: 0xf8d7e6 },
        { base: 0x4a0d2c, light: 0x8a2b5a },
        { base: 0x5c1a3d, light: 0xa35a7a },
      ];
      const color = colors[colorVariant % colors.length];
    ...
    // Create meteoroids with various sizes and positions
    const meteoroids = [];
    const meteoroidData = [
      // Large foreground meteoroids (cross over content)
      { size: 3.5, x: -18, y: 6, z: 15, speedX: 0.008, speedY: 0.006, rotSpeed: 0.01 },
      { size: 2.8, x: 20, y: -4, z: 12, speedX: -0.006, speedY: 0.008, rotSpeed: 0.012 },
      { size: 3.2, x: 5, y: 10, z: 18, speedX: 0.005, speedY: -0.007, rotSpeed: 0.008 },

      // Medium meteoroids
      { size: 2.0, x: -12, y: -8, z: 5, speedX: 0.01, speedY: 0.005, rotSpeed: 0.015 },
      { size: 1.8, x: 15, y: 8, z: 8, speedX: -0.008, speedY: 0.006, rotSpeed: 0.018 },
      { size: 2.2, x: -8, y: 12, z: 6, speedX: 0.007, speedY: -0.009, rotSpeed: 0.011 },

      // Small background meteoroids
      { size: 1.2, x: -20, y: -12, z: -5, speedX: 0.012, speedY: 0.008, rotSpeed: 0.02 },
      { size: 1.0, x: 22, y: 10, z: -8, speedX: -0.01, speedY: 0.007, rotSpeed: 0.025 },
      { size: 0.8, x: 8, y: -15, z: -10, speedX: 0.009, speedY: -0.011, rotSpeed: 0.022 },
      { size: 0.6, x: -15, y: 5, z: -12, speedX: -0.008, speedY: 0.01, rotSpeed: 0.028 },

      // Tiny distant meteoroids
      { size: 0.5, x: 25, y: -8, z: -15, speedX: 0.015, speedY: 0.012, rotSpeed: 0.03 },
      { size: 0.4, x: -25, y: 12, z: -18, speedX: -0.012, speedY: -0.008, rotSpeed: 0.035 },
      { size: 0.45, x: 12, y: 18, z: -20, speedX: 0.01, speedY: -0.015, rotSpeed: 0.032 },
    ];

    meteoroidData.forEach((data, index) => {
      const geometry = createMeteroidGeometry(data.size, 1);
      const material = createMeteroidMaterial(index);
    ...
    const animate = () => {
      meteoroidAnimationRef.current = requestAnimationFrame(animate);

      const elapsedTime = clock.getElapsedTime();

      // Animate meteoroids
      meteoroids.forEach((meteoroid) => {
        const userData = meteoroid.userData;

        // Floating motion
        const floatX = Math.sin(elapsedTime * userData.speedX * 10 + userData.floatOffset) * 3;
        const floatY = Math.cos(elapsedTime * userData.speedY * 10 + userData.floatOffset) * 2;

        meteoroid.position.x = userData.originalX + floatX;
        meteoroid.position.y = userData.originalY + floatY;

        // Rotation
        meteoroid.rotation.x += userData.rotSpeed;
        meteoroid.rotation.y += userData.rotSpeed * 0.7;
        meteoroid.rotation.z += userData.rotSpeed * 0.5;
      });
    ...
    // Handle resize
    const handleResize = () => {
      if (!meteoroidCanvasRef.current) return;
      const newWidth = meteoroidCanvasRef.current.clientWidth;
      const newHeight = meteoroidCanvasRef.current.clientHeight;
    ...
    return () => {
      ...
      if (meteoroidAnimationRef.current) {
        cancelAnimationFrame(meteoroidAnimationRef.current);
      }

      meteoroids.forEach((meteoroid) => {
        meteoroid.geometry.dispose();
        meteoroid.material.dispose();
      });
    ...
    <div className={`solar-system-container ${isAnimating ? 'expanding' : ''} ${isExpanded ? 'expanded' : 'collapsed'}`}>
      ...
      {!isExpanded && (
        <div className="solar-system-intro">
          {/* 3D Meteoroids Canvas - Three.js rendered */}
          <canvas
            ref={meteoroidCanvasRef}
            className="meteoroids-3d-canvas"
          />
